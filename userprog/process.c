...
duplicate_pte(uint64_t *pte, void *va, void *aux)
{
	...
	// 1. Check whether the VA is kernel vaddr or not.
...
__do_fork(void *aux)
{
	...
	sema_up (&current->sema_load); // sema_up 으로 parent의 sema_wait를 해제
...
int process_wait(tid_t child_tid UNUSED)
{
	...
	// struct thread *child_proc = get_child_process(child_tid);
	// if (child_proc == NULL)
	// {
	// 	return -1;
	// }
...
void process_exit(void)
{
	...
	palloc_free_multiple(curr->fd_table, FD_BASE);
	file_close(curr->file_on_run); // 현재 실행 중인 파일도 닫는다.
